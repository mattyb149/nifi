...
@Tags({"hive", "hive2", "streaming", "put", "database", "store"})
@CapabilityDescription("This processor uses Hive Streaming v1 to send flow file data to an Apache Hive 2 table. The incoming flow file is expected to be in "
...
public class PutHive2Streaming extends AbstractSessionFactoryProcessor {
    ...
    private static class FunctionContext extends RollbackOnFailure {
         ...
         * PutHive2Streaming onTrigger is not 'transactional' in a sense of RollbackOnFailure.
        ...
        private void transferFlowFiles(ProcessSession session, RoutingResult result, HiveOptions options) {
            ...
            if (successfulRecordCount.get() > 0) {
                ...
                updateAttributes.put(AbstractHive2QLProcessor.ATTR_OUTPUT_TABLES, options.getQualifiedTableName());
