...
@Tags({"put", "hive2", "ORC", "hadoop", "HDFS", "filesystem", "restricted", "record"})
@CapabilityDescription("Reads records from an incoming FlowFile using the provided Record Reader, and writes those records " +
        "to a Hive2-compatible ORC file in the location/filesystem specified in the configuration.")
...
@WritesAttributes({
        ...
        @WritesAttribute(attribute = "hive.ddl", description = "Creates a partial Hive DDL statement for creating an external table in Hive from the destination folder. "
                ...
                + "to this DDL (plus the LOCATION clause as described), then to PutHive2QL processor to create the table if it doesn't exist.")
})
...
public class PutHive2ORC extends AbstractPutHDFSRecord {
