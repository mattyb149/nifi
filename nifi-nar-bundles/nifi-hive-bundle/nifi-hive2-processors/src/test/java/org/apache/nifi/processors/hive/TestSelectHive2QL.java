...
import static org.apache.nifi.processors.hive.SelectHive2QL.HIVEQL_OUTPUT_FORMAT;
...
public class TestSelectHive2QL {
    ...
    static {
        ...
        System.setProperty("org.slf4j.simpleLogger.log.nifi.processors.hive.SelectHive2QL", "debug");
        System.setProperty("org.slf4j.simpleLogger.log.nifi.processors.hive.TestSelectHive2QL", "debug");
        LOGGER = LoggerFactory.getLogger(TestSelectHive2QL.class);
    }
    ...
    public void setup() throws InitializationException {
        ...
        runner = TestRunners.newTestRunner(SelectHive2QL.class);
        runner.addControllerService("dbcp", dbcp, dbcpProperties);
        runner.enableControllerService(dbcp);
        runner.setProperty(SelectHive2QL.HIVE2_DBCP_SERVICE, "dbcp");
    }

    @Test
    public void testIncomingConnectionWithNoFlowFile() throws InitializationException {
        runner.setIncomingConnection(true);
        runner.setProperty(SelectHive2QL.HIVEQL_SELECT_QUERY, "SELECT * FROM persons");
        runner.run();
        runner.assertTransferCount(SelectHive2QL.REL_SUCCESS, 0);
        runner.assertTransferCount(SelectHive2QL.REL_FAILURE, 0);
    }
    ...
    public void testWithNullIntColumn() throws SQLException {
        ...
        runner.setIncomingConnection(false);
        runner.setProperty(SelectHive2QL.HIVEQL_SELECT_QUERY, "SELECT * FROM TEST_NULL_INT");
        runner.run();

        runner.assertAllFlowFilesTransferred(SelectHive2QL.REL_SUCCESS, 1);
        runner.getFlowFilesForRelationship(SelectHive2QL.REL_SUCCESS).get(0).assertAttributeEquals(SelectHive2QL.RESULT_ROW_COUNT, "2");
    }
    ...
    public void testWithSqlException() throws SQLException {
        ...
        runner.setProperty(SelectHive2QL.HIVEQL_SELECT_QUERY, "SELECT val1 FROM TEST_NO_ROWS");
        runner.run();

        runner.assertAllFlowFilesTransferred(SelectHive2QL.REL_FAILURE, 1);
    }
    ...
    public void testWithBadSQL() throws SQLException {
        ...
        runner.run();
        runner.assertAllFlowFilesTransferred(SelectHive2QL.REL_FAILURE, 1);
        MockFlowFile flowFile = runner.getFlowFilesForRelationship(SelectHive2QL.REL_FAILURE).get(0);
        ...
        runner.setIncomingConnection(false);
        // Try a valid SQL statement that will generate an error (val1 does not exist, e.g.)
        runner.setProperty(SelectHive2QL.HIVEQL_SELECT_QUERY, BAD_SQL);
        runner.run();
        runner.assertAllFlowFilesTransferred(SelectHive2QL.REL_FAILURE, 1);
        flowFile = runner.getFlowFilesForRelationship(SelectHive2QL.REL_FAILURE).get(0);
    ...
    public void invokeOnTrigger(final String query, final boolean incomingFlowFile, String outputFormat,
        ...
        final List<MockFlowFile> flowfiles = runner.getFlowFilesForRelationship(SelectHive2QL.REL_SUCCESS);
        ...
        assertEquals(recordsFromStream, Integer.parseInt(flowFile.getAttribute(SelectHive2QL.RESULT_ROW_COUNT)));
        flowFile.assertAttributeEquals(AbstractHive2QLProcessor.ATTR_INPUT_TABLES, "persons");
    }
    ...
    public TestRunner doOnTrigger(final String query, final boolean incomingFlowFile, String outputFormat,
        ...
        runner.setProperty(SelectHive2QL.HIVEQL_SELECT_QUERY, query);
        runner.setProperty(HIVEQL_OUTPUT_FORMAT, outputFormat);
        if (preQueries != null) {
            runner.setProperty(SelectHive2QL.HIVEQL_PRE_QUERY, preQueries);
        }
        if (postQueries != null) {
            runner.setProperty(SelectHive2QL.HIVEQL_POST_QUERY, postQueries);
        }
    ...
    public void testMaxRowsPerFlowFileAvro() throws ClassNotFoundException, SQLException, InitializationException, IOException {
        ...
        runner.setIncomingConnection(false);
        runner.setProperty(SelectHive2QL.HIVEQL_SELECT_QUERY, "SELECT * FROM TEST_QUERY_DB_TABLE");
        runner.setProperty(SelectHive2QL.MAX_ROWS_PER_FLOW_FILE, "${" + MAX_ROWS_KEY + "}");
        runner.setProperty(SelectHive2QL.HIVEQL_OUTPUT_FORMAT, HiveJdbcCommon.AVRO);
        runner.setVariable(MAX_ROWS_KEY, "9");

        runner.run();
        runner.assertAllFlowFilesTransferred(SelectHive2QL.REL_SUCCESS, 12);

        //ensure all but the last file have 9 records each
        for (int ff = 0; ff < 11; ff++) {
            mff = runner.getFlowFilesForRelationship(SelectHive2QL.REL_SUCCESS).get(ff);
        ...
        mff = runner.getFlowFilesForRelationship(SelectHive2QL.REL_SUCCESS).get(11);
        in = new ByteArrayInputStream(mff.toByteArray());
    ...
    public void testParametrizedQuery() throws ClassNotFoundException, SQLException, InitializationException, IOException {
        ...
        runner.run();
        runner.assertAllFlowFilesTransferred(SelectHive2QL.REL_SUCCESS, 1);
        MockFlowFile flowFile = runner.getFlowFilesForRelationship(SelectHive2QL.REL_SUCCESS).get(0);
    ...
    public void testMaxRowsPerFlowFileCSV() throws ClassNotFoundException, SQLException, InitializationException, IOException {
        ...
        runner.setIncomingConnection(true);
        runner.setProperty(SelectHive2QL.MAX_ROWS_PER_FLOW_FILE, "${" + MAX_ROWS_KEY + "}");
        runner.setProperty(SelectHive2QL.HIVEQL_OUTPUT_FORMAT, HiveJdbcCommon.CSV);
    ...
    public void testMaxRowsPerFlowFileWithMaxFragments() throws ClassNotFoundException, SQLException, InitializationException, IOException {
        ...
        runner.setIncomingConnection(false);
        runner.setProperty(SelectHive2QL.HIVEQL_SELECT_QUERY, "SELECT * FROM TEST_QUERY_DB_TABLE");
        runner.setProperty(SelectHive2QL.MAX_ROWS_PER_FLOW_FILE, "9");
        Integer maxFragments = 3;
        runner.setProperty(SelectHive2QL.MAX_FRAGMENTS, maxFragments.toString());

        runner.run();
        runner.assertAllFlowFilesTransferred(SelectHive2QL.REL_SUCCESS, maxFragments);

        for (int i = 0; i < maxFragments; i++) {
            mff = runner.getFlowFilesForRelationship(SelectHive2QL.REL_SUCCESS).get(i);
            in = new ByteArrayInputStream(mff.toByteArray());
            assertEquals(9, getNumberOfRecordsFromStream(in));
    ...
    /**
     * Simple implementation only for SelectHive2QL processor testing.
     */
