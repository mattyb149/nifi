...
public class PutHive2ORCTest {
    ...
    public void setup() throws IOException {
        ...
        proc = new PutHive2ORC();
    }

    private void configure(final PutHive2ORC putORC, final int numUsers) throws InitializationException {
        configure(putORC, numUsers, null);
    }

    private void configure(final PutHive2ORC putORC, final int numUsers, final BiFunction<Integer, MockRecordParser, Void> recordGenerator) throws InitializationException {
        testRunner = TestRunners.newTestRunner(putORC);
        testRunner.setProperty(PutHive2ORC.HADOOP_CONFIGURATION_RESOURCES, TEST_CONF_PATH);
        testRunner.setProperty(PutHive2ORC.DIRECTORY, DIRECTORY);
        ...
        testRunner.setProperty(PutHive2ORC.RECORD_READER, "mock-reader-factory");
    }
    ...
    public void testWriteORCWithDefaults() throws IOException, InitializationException {
        ...
        testRunner.setProperty(PutHive2ORC.HIVE_TABLE_NAME, "myTable");

        testRunner.enqueue("trigger", flowFileAttributes);
        testRunner.run();
        testRunner.assertAllFlowFilesTransferred(PutHive2ORC.REL_SUCCESS, 1);

        final Path orcFile = new Path(DIRECTORY + "/" + filename);

        // verify the successful flow file has the expected attributes
        final MockFlowFile mockFlowFile = testRunner.getFlowFilesForRelationship(PutHive2ORC.REL_SUCCESS).get(0);
        mockFlowFile.assertAttributeEquals(PutHive2ORC.ABSOLUTE_HDFS_PATH_ATTRIBUTE, orcFile.getParent().toString());
        mockFlowFile.assertAttributeEquals(CoreAttributes.FILENAME.key(), filename);
        mockFlowFile.assertAttributeEquals(PutHive2ORC.RECORD_COUNT_ATTR, "100");
        mockFlowFile.assertAttributeEquals(PutHive2ORC.HIVE_DDL_ATTRIBUTE,
    ...
    public void testWriteORCWithAvroLogicalTypes() throws IOException, InitializationException {
        ...
        mockFlowFile.assertAttributeEquals(PutHive2ORC.ABSOLUTE_HDFS_PATH_ATTRIBUTE, orcFile.getParent().toString());
        mockFlowFile.assertAttributeEquals(CoreAttributes.FILENAME.key(), filename);
        mockFlowFile.assertAttributeEquals(PutHive2ORC.RECORD_COUNT_ATTR, "10");
        ...
        verifyORCUsers(orcFile, 10, (x, currUser) -> {
                    ...
                    assertEquals(noTimeOfDayDateFormat.format(dt), noTimeOfDayDateFormat.format(((DateWritable) x.get(3)).get()));
    ...
    public void testMalformedRecordExceptionFromReaderShouldRouteToFailure() throws InitializationException, IOException, MalformedRecordException, SchemaNotFoundException {
        ...
        testRunner.enqueue("trigger", flowFileAttributes);
        testRunner.run();
        testRunner.assertAllFlowFilesTransferred(PutHive2ORC.REL_FAILURE, 1);
    }

    @Test
    public void testIOExceptionCreatingWriterShouldRouteToRetry() throws InitializationException {
        final PutHive2ORC proc = new PutHive2ORC() {
            @Override
        ...
        testRunner.enqueue("trigger", flowFileAttributes);
        testRunner.run();
        testRunner.assertAllFlowFilesTransferred(PutHive2ORC.REL_RETRY, 1);
    }
