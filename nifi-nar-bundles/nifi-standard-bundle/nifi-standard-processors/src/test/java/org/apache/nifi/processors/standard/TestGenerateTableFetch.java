...
public class TestGenerateTableFetch {
    ...
    @Test
    public void getSelectFromClauseWithFilter() throws Exception {
        assertEquals("SELECT * FROM myTable WHERE id > 2", dbAdapter.getSelectStatement("myTable", null, "id > 2", null, null, null));
        assertEquals("SELECT myCol1, myCol2 FROM myTable WHERE myCol1 = 'Hello' AND myCol2 < 4",
                dbAdapter.getSelectStatement("myTable", "myCol1, myCol2", "myCol1 = 'Hello' AND myCol2 < 4", null, null, null));
        assertEquals("SELECT * FROM myTable WHERE id > 2 LIMIT 10", dbAdapter.getSelectStatement("myTable", null, "id > 2", null, 10, null));
        assertEquals("SELECT * FROM myTable WHERE id < 4 OFFSET 10", dbAdapter.getSelectStatement("myTable", null, "id < 4", null, null, 10));
        assertEquals("SELECT col1 FROM myTable WHERE id < 4 LIMIT 2 OFFSET 10", dbAdapter.getSelectStatement("myTable", "col1", "id < 4", null, 2, 10));
    }
    ...
    @Test
    public void getSelectFromClauseOracleWithFilter() throws Exception {
        dbAdapter = new OracleDatabaseAdapter();
        assertEquals("SELECT * FROM myTable WHERE id > 2",
                dbAdapter.getSelectStatement("myTable", null, "id > 2", null, null, null));
        assertEquals("SELECT myCol1, myCol2 FROM myTable WHERE id > 2",
                dbAdapter.getSelectStatement("myTable", "myCol1, myCol2", "id > 2", null, null, null));
        assertEquals("SELECT * FROM (SELECT a.*, ROWNUM rnum FROM (SELECT * FROM myTable WHERE id > 2) a WHERE ROWNUM <= 10) WHERE rnum > 0",
                dbAdapter.getSelectStatement("myTable", null, "id > 2", null, 10, null));
    }
    ...
    @Test
    public void testOnTriggerExtraPartitionWithMaxValues() throws Exception {
        runner.setProperty(GenerateTableFetch.PARTITION_SIZE, "3");
        runner.enqueue(new byte[0], new HashMap<String, String>() {{
            put("db.table.fullname", "person");
            put("db.table.count", "100");
            put(ListDatabaseTables.DB_TABLE_MAXVAL_PREFIX + "id.type", "4");
            put(ListDatabaseTables.DB_TABLE_MAXVAL_PREFIX + "id.value", "102");
        }});

        runner.run();
        runner.assertTransferCount(GenerateTableFetch.REL_SUCCESS, 34);
        MockFlowFile flowFile = runner.getFlowFilesForRelationship(GenerateTableFetch.REL_SUCCESS).get(0);
        assertEquals("SELECT * FROM person WHERE id > 102 LIMIT 3 OFFSET 0", new String(flowFile.toByteArray()));
        runner.assertTransferCount(GenerateTableFetch.REL_ORIGINAL, 1);
    }
    ...
    @Test
    public void testOnTriggerFetchAllWithMaxValues() throws Exception {
        runner.setProperty(GenerateTableFetch.PARTITION_SIZE, "0");
        runner.setProperty(GenerateTableFetch.COLUMN_NAMES, "id, col1");
        runner.enqueue(new byte[0], new HashMap<String, String>() {{
            put("db.table.fullname", "person");
            put("db.table.count", "100");
            put(ListDatabaseTables.DB_TABLE_MAXVAL_PREFIX + "id.type", "4");
            put(ListDatabaseTables.DB_TABLE_MAXVAL_PREFIX + "id.value", "10");
        }});
        ...
        MockFlowFile resultFlowFile = runner.getFlowFilesForRelationship(GenerateTableFetch.REL_SUCCESS).get(0);
        assertNotNull(resultFlowFile);
        resultFlowFile.assertContentEquals("SELECT id, col1 FROM person WHERE id > 10");
    }
